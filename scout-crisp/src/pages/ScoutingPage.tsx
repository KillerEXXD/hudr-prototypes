    ...
    <div className="animate-fade-up">
        ...
        <div className="mt-3 flex items-start gap-2 rounded-xl border border-accent-amber/30 bg-accent-amber/10 p-3">
          ...
          <p className="text-xs leading-snug text-text-secondary">
            {isPro
              ...
              : 'Small sample — early hints only. Try Scope: Career.'}
          </p>
        </div>
      )}
      ...
      <Section icon={<Brain className="h-4 w-4" />} title={isPro ? 'Player type' : 'How they play'}>
        <div className="rounded-xl border border-border bg-bg-card p-4">
          {/* Crisp Amateur: archetype phrase only — drop the confidence-caveat sentence. */}
          <p className="text-sm leading-relaxed text-text-primary">{isPro ? narrative.proSummary : narrative.typePlain.split('. ')[0] + '.'}</p>
          {dims && (
      ...
      <Section
        ...
        sub={isPro ? 'Only RELIABLE-tier leaks fire. Each pairs a counter with a confirmation stat.' : undefined}
      >
        {exploits.length > 0 ? (
      ...
      <Section icon={<Award className="h-4 w-4" />} title={isPro ? 'Strengths' : 'What they do well'}>
        <Strengths strengths={profile.strengths} plain={!isPro} />
      </Section>

      {/* ---- Layer 4: exploit plan (by phase) ---- */}
      <Section icon={<ClipboardList className="h-4 w-4" />} title={isPro ? 'Exploit plan' : 'Your game plan'} sub={isPro ? 'Actionable, by phase.' : undefined}>
