...
import type { ApiTournament, ApiPlayer, ApiPlayerStats, ApiHighlight, ApiHand, ApiSuggestedQuestion, ApiPlayerTournament, ApiCurrentUser, ApiSubscriptionPlan, ApiTrendingQuery, ApiStatHand } from '../types'
...
// ---- Hands behind a specific stat (mirrors hudr-pwa /player-stat-hands) ----
const HOLE_CARDS = ['As Ks', 'Qh Qd', 'Jc Jh', 'Ad 8d', 'Tc Th', '9s 9h', 'Kd Qc', '7h 7s', 'Ah Kh', '5c 5d', 'Js Ts', '8c 8h', 'Ac Qd', '6h 6s', 'Kh Jh']
const POSITIONS = ['UTG', 'HJ', 'CO', 'BTN', 'SB', 'BB']
// engine StatKey → the % field on ApiPlayerStats (used to bias the Executed/Folded split).
// af / afq / gap are ratios/derived → no % → default ~50% executed.
const STAT_FIELD: Partial<Record<string, keyof ApiPlayerStats>> = {
  vpip: 'vpip', pfr: 'pfr', threeBet: 'three_bet', foldTo3Bet: 'fold_to_3bet', fourBet: 'four_bet',
  coldCall: 'cold_call', steal: 'steal', foldToSteal: 'fold_to_steal', cbetFlop: 'cbet_flop',
  cbetTurn: 'cbet_turn', foldToCbetFlop: 'fold_to_cbet_flop', foldToCbetTurn: 'fold_to_cbet_turn',
  checkRaise: 'check_raise', donk: 'donk', wtsd: 'wtsd', wsd: 'wsd', wwsf: 'wwsf', riverBet: 'river_bet',
}

export function buildStatHands(playerId: string, statType: string): ApiStatHand[] {
  const stats = apiPlayerStats[playerId]
  const field = STAT_FIELD[statType]
  const pct = field && stats ? Number(stats[field]) : NaN
  // Executed fraction tracks how often the player takes the action this stat measures.
  const frac = Number.isFinite(pct) ? Math.min(0.85, Math.max(0.15, pct / 100)) : 0.5
  const total = 14
  const seed = (playerId.charCodeAt(1) || 1) * 13 + statType.length * 7
  const executed = Math.max(2, Math.min(total - 2, Math.round(total * frac)))
  return Array.from({ length: total }, (_, i) => {
    const j = (i * 3 + seed) % BOARDS.length
    const win = (i + seed) % 5 < 3
    const mag = 500000 + ((i * 137 + seed) % 9) * 850000
    return {
      hand_number: 90 - i - (seed % 7),
      stat_type: statType,
      action_taken: i < executed,
      position: POSITIONS[(i + seed) % POSITIONS.length],
      hole_cards: HOLE_CARDS[(i + seed) % HOLE_CARDS.length].split(' '),
      board: BOARDS[j].split(' '),
      pot: 250000 + ((i * 5 + seed) % 8) * 600000,
      result: RESULTS[j % RESULTS.length],
      net_result: (win ? 1 : -1) * mag,
      video_seconds: 120 + ((i * 53 + seed) % 60) * 41,
      video_url: `${YT_BASE}${120 + i * 41}`,
      has_replay: true,
    }
  })
}
