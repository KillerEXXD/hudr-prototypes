...
// One hand behind a specific stat (mirrors hudr-pwa /player-stat-hands).
export interface ApiStatHand {
  hand_number: number
  stat_type: string
  action_taken: boolean       // true = did the action (Executed); false = faced it & passed (Folded)
  position: string
  hole_cards: string[]
  board: string[]
  pot: number
