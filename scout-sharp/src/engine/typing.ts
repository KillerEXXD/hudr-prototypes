...
export function classifyPlayer(stats: StatWithTier[], boundaries: Boundary[] = ARCHETYPE_BOUNDARIES): TypingResult {
  ...
  // Conservative confidence: each typing input contributes how *close* its own
  // opportunity count is to a RELIABLE sample (0..1), not a binary reliable flag.
  // Averaged, then capped at 0.9 — a model can never be 100% sure of a human's
  // type, and certainly not off a handful of hands. This is what makes
  // "100% confidence over 72 hands" impossible by construction.
  const sufficiency = (k: StatKey): number => {
    const s = stats.find((x) => x.key === k)
    if (!s) return 0
    const reliableAt = TIER_THRESHOLDS[s.category].reliable
    return Math.max(0, Math.min(1, s.opportunities / reliableAt))
  }
  const meanSuff = TYPING_INPUTS.reduce((acc, k) => acc + sufficiency(k), 0) / TYPING_INPUTS.length
  const confidence = Math.min(0.9, meanSuff * 0.9)
