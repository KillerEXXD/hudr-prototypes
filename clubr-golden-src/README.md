# Clubr · "Felt" variant — the reference-faithful, installable PWA

This variant reproduces the **canonical ClubrGo reference design** (the kit's
...
- `display: standalone`, `orientation: portrait`, scoped to `/clubr-golden/`.
...
## Run
```bash
cd clubr-golden-src && npm install && npm run build   # → ../clubr-golden/
```
`base:'/clubr-golden/'`, `outDir:'../clubr-golden'`. Tailwind v4 (CSS-first).
