...
import { captureReview, getIdentity, setIdentity, isEmail } from '@/lib/analytics'
import { getReviewSections } from '@/lib/reviewSections'
import { cn } from '@/lib/utils/cn'

// Guided, feature-by-feature review of the ClubR prototype. Identity (name +
// email) is captured FIRST and reused across the short + long feedback. Then
// one step per ClubR feature area (score 1–5 + liked/disliked chips + free
// text, with a "try it" deep link), ending in an overall step. Submits one
// structured PostHog event.
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              <div className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-accent-blue">
                <ClipboardList className="h-3.5 w-3.5" /> Review · ClubR
              </div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin p-4">
          {sent ? (
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              ...
              <p className="text-xs text-text-muted">This directly shapes the ClubR build.</p>
            </div>
          ) : isIdentity ? (
            <div>
              <h3 className="text-base font-bold text-text-primary">Quick ClubR review</h3>
              <p className="mt-0.5 text-xs leading-snug text-text-muted">~3 minutes — you’ll rate each feature (Discover, joining a club, FT Fantasy, Last Longer, hosting…) then a couple of overall questions. First, who are you?</p>
