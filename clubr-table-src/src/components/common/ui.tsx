...
export function Card({ children, className, onClick, id }: { children: ReactNode; className?: string; onClick?: () => void; id?: string }) {
  return (
    <div
      id={id}
      onClick={onClick}
      className={cn('rounded-3xl border border-border bg-bg-card p-5', onClick && 'cursor-pointer transition-all hover:border-border-light active:scale-[0.99]', className)}
    >
      {children}
    </div>
  )
...
export function Btn({ children, onClick, variant = 'primary', size = 'md', disabled, loading, className, type = 'button' }: {
  ...
  const variants: Record<string, string> = {
    primary: 'bg-accent-gold text-bg-primary hover:brightness-105 font-bold shadow-[0_8px_28px_-10px_rgba(52,232,158,0.55)]',
  ...
  return (
    <section className={cn('mt-5', className)}>
      <div className="mb-2 flex items-center justify-between px-0.5">
        <h2 className="text-base font-extrabold tracking-tight text-text-secondary">{title}</h2>
        {action}
      </div>
      {children}
    </section>
