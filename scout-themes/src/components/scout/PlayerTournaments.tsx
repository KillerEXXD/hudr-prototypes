...
import { Trophy, X, Loader2, ChevronRight } from 'lucide-react'
import { usePlayerTournaments } from '@/hooks'

// Button + centered dialog listing the tournaments analysed for this player,
// with hands played per tournament. Each row links to the tournament page.
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={() => setOpen(false)}>
          <div className="animate-fade-up flex max-h-[80vh] w-full max-w-sm flex-col rounded-2xl border border-border bg-bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-3 border-b border-border p-4">
              <div>
                <h3 className="text-sm font-bold text-text-primary">Tournaments analysed</h3>
                <p className="text-xs text-text-muted">
                  {isLoading ? 'Loading…' : <>{firstName} · {tournaments.length} tournaments · <span className="nums">{totalHands.toLocaleString()}</span> hands</>}
                </p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin p-3">
              {isLoading ? (
                ...
                <div className="py-8 text-center text-sm text-text-muted">No tournaments analysed yet.</div>
              ) : (
                <div className="space-y-1.5">
                  {tournaments.map((t) => (
                    <Link
                      ...
                      className="flex items-center justify-between gap-3 rounded-lg border border-border bg-bg-surface/40 px-3 py-2 transition-colors hover:border-border-light hover:bg-bg-surface cursor-pointer"
                    >
