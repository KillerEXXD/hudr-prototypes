...
import { ShieldCheck, Crown, User as UserIcon, LogOut, ChevronRight, Palette, Coins, Pencil, MapPin, GalleryHorizontalEnd, Camera, Check, Ban, Sparkles, Trash2 } from 'lucide-react'
  ...
  return (
    <div className="animate-fade-up">
      ...
      <Section title="Account">
        <Card onClick={() => setDeleteOpen(true)} className="flex items-center gap-3 border-accent-red/20 bg-accent-red/5 hover:bg-accent-red/10">
          <Trash2 className="h-5 w-5 text-accent-red" />
          <div className="flex-1">
            <p className="text-sm font-bold text-accent-red">Delete account</p>
            <p className="text-xs text-text-muted">Anonymize PII immediately · 14-day grace window before hard-purge</p>
          </div>
          <ChevronRight className="h-4 w-4 text-text-muted" />
        </Card>
      </Section>
      <DeleteAccountSheet open={deleteOpen} onClose={() => setDeleteOpen(false)} user={user} onSuccess={() => { logout(); navigate('/') }} />
