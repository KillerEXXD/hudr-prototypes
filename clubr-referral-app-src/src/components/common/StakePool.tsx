...
export function StakePool({ stake, pool, children, right }: { stake: number; pool: number; children?: ReactNode; right?: ReactNode }) {
  return (
    <div className="mt-2 flex items-center gap-1.5 text-[11px]">
      <span className="inline-flex items-center gap-1 rounded-lg border border-border bg-bg-surface/60 px-1.5 py-1 font-semibold text-text-secondary" title="Buy-in per entry">
        <Ticket className="h-3 w-3 text-accent-amber" /><span className="font-mono">{stake}</span> buy-in
      </span>
      <span className="inline-flex items-center gap-1 rounded-lg border border-accent-emerald/30 bg-accent-emerald/10 px-1.5 py-1 font-bold text-accent-emerald" title="Current prize pool (buy-in × entrants)">
