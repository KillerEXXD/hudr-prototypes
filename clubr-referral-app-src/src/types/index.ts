...
export interface User {
  ...
  /** Set when the user initiates account deletion. 14-day grace window — logging
   *  in any time before `deletedAt + 14d` cancels the deletion. After 14d a cron
   *  job hard-purges the row. */
  deleteRequestedAt?: string
  /** Set when the user is anonymized (PII wiped, name → "Removed Player"). The
   *  account row stays so historical attributions still resolve. */
  deletedAt?: string
...
export interface Club {
  ...
  /** The founder. Set at club creation and NEVER changes. Always also an
   *  Owner. Only the Creator can add/remove Owners and Dissolve the club.
   *  Distinct from ownerId (which is the "primary owner" pointer; can rotate). */
  creatorId: string
  ownerId: string
  ownerName: string
  inviteCode: string
  createdAt: string
  members: ClubMember[]
  /** Set when the Creator Dissolves the club. Read-only across surfaces after this.
   *  Past games still resolve via member personal histories with a "(dissolved)" tag. */
  dissolvedAt?: string
  dissolvedBy?: string
