...
export function ClubRow({ club, right }: { club: ClubView; right?: React.ReactNode }) {
  ...
  return (
    <div>
      <Card onClick={() => navigate(`/club/${club.id}`)} className="flex items-center gap-3.5 p-3.5">
        <Avatar emoji={club.emoji} color={club.color} size={52} />
        <div className="min-w-0 flex-1">
          <p className="flex items-center gap-1.5 text-[16px] font-bold text-text-primary" style={{ fontFamily: 'var(--font-family-display)' }}>
          ...
          <p className="mt-0.5 truncate text-[13px] text-text-muted">{club.location ? <><MapPin className="mr-0.5 inline h-3 w-3" />{club.location} · </> : null}<Users className="mr-1 inline h-3 w-3" />{memberCount} members</p>
        </div>
