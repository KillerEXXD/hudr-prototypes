...
// computed profile (the narrator never invents numbers).
...
// Focused, near-full-screen chat. The composer is the LAST child so it docks at
// the bottom of the screen (above the keyboard); the overlay covers the bottom nav.
export default function PlayerChatSheet({ open, onClose, profile }: { open: boolean; onClose: () => void; profile: PlayerProfile }) {
  const { isPro } = useMode()
  ...
  function ask(q: string) {
    ...
    // Don't focus the input here — on iOS that pops the keyboard on a suggestion tap.
  }
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2 border-b border-border p-3">
          <div className="flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-accent-blue/15 text-accent-blue"><Sparkles className="h-4 w-4" /></span>
            <div>
              <div className="text-sm font-bold text-text-primary">Ask AI about {first}</div>
              <div className="text-[11px] text-text-muted">{isPro ? 'Answers from computed stats & exploits.' : 'Plain-English reads on how to beat them.'}</div>
            </div>
          </div>
