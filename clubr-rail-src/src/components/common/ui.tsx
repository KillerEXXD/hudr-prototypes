...
export function Card({ children, className, onClick, id }: { children: ReactNode; className?: string; onClick?: () => void; id?: string }) {
  return (
    <div
      id={id}
      onClick={onClick}
      className={cn('rounded-[20px] border border-border bg-bg-card p-4', onClick && 'cursor-pointer transition-all hover:border-border-light active:scale-[0.995]', className)}
    >
      {children}
    </div>
  )
...
export function Btn({ children, onClick, variant = 'primary', size = 'md', disabled, loading, className, type = 'button' }: {
  ...
  const variants: Record<string, string> = {
    primary: 'bg-accent-gold text-white hover:brightness-105 shadow-[0_8px_24px_-12px_rgba(255,122,89,0.5)]',
  ...
  return (
    <section className={cn('mt-5', className)}>
      <div className="mb-2 flex items-center justify-between px-0.5">
        <h2 className="text-[13px] font-bold tracking-tight text-text-secondary">{title}</h2>
        {action}
      </div>
      {children}
    </section>
