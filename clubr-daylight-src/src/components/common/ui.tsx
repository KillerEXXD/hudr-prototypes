...
export function Badge({ children, tone = 'neutral', className }: { children: ReactNode; tone?: 'neutral' | 'blue' | 'green' | 'amber' | 'red' | 'purple' | 'dark' | 'gold'; className?: string }) {
  const tones: Record<string, string> = {
    ...
    dark: 'bg-text-primary text-white border-transparent',
  }
...
export function Card({ children, className, onClick, id }: { children: ReactNode; className?: string; onClick?: () => void; id?: string }) {
  return (
    <div
      id={id}
      onClick={onClick}
      className={cn('rounded-2xl border border-border bg-bg-card p-4 shadow-[0_2px_10px_-6px_rgba(30,42,36,0.12)]', onClick && 'cursor-pointer transition-all hover:shadow-[0_8px_24px_-10px_rgba(30,42,36,0.18)] hover:-translate-y-px active:translate-y-0', className)}
    >
      {children}
    </div>
  )
...
export function Btn({ children, onClick, variant = 'primary', size = 'md', disabled, loading, className, type = 'button' }: {
  ...
  const variants: Record<string, string> = {
    primary: 'bg-accent-emerald text-white hover:brightness-105 shadow-[0_6px_18px_-8px_rgba(47,111,79,0.5)]',
    secondary: 'bg-bg-card text-text-primary border border-border-light hover:bg-bg-surface',
  ...
  return (
    <section className={cn('mt-6', className)}>
      <div className="mb-2.5 flex items-center justify-between px-0.5">
        <h2 className="text-[15px] font-semibold tracking-tight text-text-primary" style={{ fontFamily: 'var(--font-family-display)' }}>{title}</h2>
        {action}
      </div>
      {children}
    </section>
...
export function EmptyState({ icon, title, sub }: { icon?: ReactNode; title: string; sub?: string }) {
  return (
    <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-border-light bg-bg-card px-6 py-10 text-center shadow-[0_2px_10px_-6px_rgba(30,42,36,0.1)]">
