    ...
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" className={className} aria-hidden="true">
      <defs>
        <linearGradient id={id} x1="6" y1="4" x2="42" y2="44" gradientUnits="userSpaceOnUse">
          <stop stopColor="#2F6F4F" />
          <stop offset="1" stopColor="#235740" />
        </linearGradient>
      </defs>
...
export function LogoLockup({ size = 22, wordClass = 'text-lg', className }: { size?: number; wordClass?: string; className?: string }) {
  return (
    <span className={`inline-flex items-center gap-2 ${className ?? ''}`}>
      <LogoMark size={size} />
      <span className={`font-bold tracking-tight text-text-primary ${wordClass}`} style={{ fontFamily: 'var(--font-family-display)' }}>Clubr<span className="text-accent-gold">Go</span></span>
    </span>
  )
}
