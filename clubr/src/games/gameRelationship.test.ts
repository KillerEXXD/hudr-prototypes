...
const g = (over: Partial<UnifiedGame>) =>
  ({ type: 'last_longer', id: 'x', clubId: 'c', canManage: false, finished: false, phase: 'reg', mine: false, sort: 0, ...over } as unknown as UnifiedGame)

describe('relationshipOf', () => {
  it('hosting when you manage it (even if you also play it)', () => {
    expect(relationshipOf(g({ canManage: true }))).toBe('hosting')
    expect(relationshipOf(g({ canManage: true, mine: true, phase: 'live' }))).toBe('hosting')
  })
  it('playing when you joined it and do not manage it', () => {
