...
export function useCreateGame() { const { userId } = useIdentity(); const inv = useInvalidate(); return useMutation({ mutationFn: (v: { clubId: string; title: string; location: string; mode: 'in-person' | 'online'; stake: number; visibility: 'public' | 'private'; accessUserIds: string[] }) => ll.createGame(v.clubId, userId, { title: v.title, location: v.location, mode: v.mode, stake: v.stake, visibility: v.visibility, accessUserIds: v.accessUserIds }), onSuccess: inv }) }
