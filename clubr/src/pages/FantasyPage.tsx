...
import { Target, Clock, Lock, CheckCircle2, Eye, Shield, Plus } from 'lucide-react'
...
export function FantasyPage() {
  ...
  const hosting = data?.filter((c) => c.canManage) ?? []
  const playing = data?.filter((c) => !c.canManage) ?? []
        ...
        <>
          {hosting.length > 0 && (
            <Section title="You're hosting">
              <div className="flex flex-col gap-2">{hosting.map((c) => <ContestRow key={c.id} c={c} />)}</div>
            </Section>
          )}
          <Section title={hosting.length > 0 ? 'Playing in' : 'Contests in your clubs'}>
            {playing.length > 0 ? (
              <div className="flex flex-col gap-2">{playing.map((c) => <ContestRow key={c.id} c={c} />)}</div>
            ) : (
              <EmptyState title="Nothing to play in yet" sub="Contests you're a player in show here." />
            )}
          </Section>
        </>
      ) : (
        <Section title="Contests in your clubs">
          <EmptyState icon={<Target className="h-7 w-7" />} title="No contests yet" sub="Join a club to see its FT Fantasy contests, or host one above." />
        </Section>
      )}
