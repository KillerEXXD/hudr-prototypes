...
export function Avatar({ name, color, size = 36, emoji }: { name?: string; color?: string; size?: number; emoji?: string }) {
  ...
  return (
    <span
      ...
      style={{ width: size, height: size, background: color ?? '#6b7280', fontSize: size * 0.4, fontFamily: 'var(--font-family-display)' }}
    >
      {emoji ?? initials}
    </span>
  )
...
export function Card({ children, className, onClick, id }: { children: ReactNode; className?: string; onClick?: () => void; id?: string }) {
  return (
    <div
      id={id}
      onClick={onClick}
      className={cn('rounded-2xl border border-border bg-bg-card p-4 shadow-[0_6px_20px_-14px_rgba(0,0,0,0.7)]', onClick && 'cursor-pointer transition-all hover:border-border-light active:scale-[0.995]', className)}
    >
      {children}
    </div>
  )
...
export function Btn({ children, onClick, variant = 'primary', size = 'md', disabled, loading, className, type = 'button' }: {
  ...
  const variants: Record<string, string> = {
    // Felt signature: the primary CTA is chip-gold with dark ink (not blue/white).
  ...
  return (
    <section className={cn('mt-5', className)}>
      <div className="mb-2 flex items-center justify-between px-0.5">
        <h2 className="font-mono text-[11px] font-semibold uppercase tracking-[0.12em] text-text-muted">{title}</h2>
        {action}
      </div>
      {children}
    </section>
