...
export function LogoLockup({ size = 22, wordClass = 'text-lg', className }: { size?: number; wordClass?: string; className?: string }) {
  return (
    <span className={`inline-flex items-center gap-2 ${className ?? ''}`}>
      <LogoMark size={size} />
      <span className={`font-extrabold tracking-tight text-text-primary ${wordClass}`} style={{ fontFamily: 'var(--font-family-display)' }}>Clubr<span className="text-accent-gold">GO</span></span>
    </span>
  )
}
