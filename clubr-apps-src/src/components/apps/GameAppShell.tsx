...
import { Trophy, Clock, Grid3x3, ChevronRight, Sparkles, ArrowLeft } from 'lucide-react'
 ...
 *  ┌─────────────────────────────────────┐
 *  │  ← Home                              │
 *  │  [icon]  Game Name  [?]              │  ← '?' opens How-it-works sheet
 *  │          sub-line                    │
 *  ├─────────────────────────────────────┤
 *  │  New · Live · Finished               │  ← 3 tabs only ('How' removed)
 *  ├─────────────────────────────────────┤
 *  │  [list of games filtered by tab]    │
 *  └─────────────────────────────────────┘
 *
 * 'How it works' moved out of the tab row and into a "?" button next to the
 * game name in the header — mirrors the live ClubR app pattern.
 */
  ...
  return (
    <div className="relative animate-fade-up">
      ...
      {rows.length > 0 ? (
        ...
        <EmptyState>
          {tab === 'new' && (emptyOpen ?? <>No new {app.label} games right now. The next one will appear here.</>)}
      ...
      {/* How-it-works sheet — opened from the '?' next to the title. */}
      <Sheet open={howOpen} onClose={() => setHowOpen(false)} title={`How ${app.label} works`}>
        <div className="text-sm leading-relaxed text-text-secondary">{howItWorks}</div>
      </Sheet>
    </div>
  )
}
